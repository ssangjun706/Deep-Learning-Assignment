...
class nn_convolutional_layer:
    ...
    def convolve(self, x):
        y = view_as_windows(x, (1, 3, 3)).reshape(26, 26, -1)
        W  = self.W.reshape((28, -1, 1))
        b = self.b.reshape((-1, 1))
        return torch.stack([(torch.matmul(y, W[j]) + b[j]).squeeze() for j in range(28)])
    ...
    def forward(self, x):
        ...
        ###################################
        return torch.stack([self.convolve(x[i]) for i in range(x.shape[0])])
...
class nn_max_pooling_layer:
    ...
    def pool(self, x):
        pool = self.pool_size
        stride = self.stride
        conv = [view_as_windows(x[i], (pool, pool), step=stride) for i in range(28)]
        return torch.stack([torch.amax(y.reshape((13, 13, -1)), 2) for y in conv])
    ...
    def forward(self, x):
        ...
        ###################################
        return torch.stack([self.pool(x[i]) for i in range(x.shape[0])])
